...
class GoGo(scrapy.Spider):
    ...
    def zh(self, response):
        ...
        user_expectsalary = user_expectsalary[:-3]

        #毕业院校
        user_school = response.xpath("//div[@class='panel proginn-work-history'][last()]//p[@class='title']//span[2]//text()").extract()
        user_school = ','.join(user_school)
        ...
        #爬取源链接
        source_link = response.xpath("//head//link[@rel='canonical']//@href").extract()
        ...
        #姓名
        user_name = response.xpath("//a[@class='header']//text()").extract()
        ...
        #源数据id
        source_id = response.xpath("//head//link[@rel='canonical']//@href").extract()[0]
        source_id = source_id[27:]

        #信息来源
        user_consultingsources = '程序员客栈'
        ...
        item = ForprogrammerItem(
            ...
            source_id = source_id,
            user_consultingsources = user_consultingsources
        )
